...
// لا تستعمل useMesAnnonces() لتجنب تعقيدات الجلب
export default function ProprietaireContratsPage() {
  ...
  useEffect(() => {
    ...
    setLoadingData(true);
    Promise.all([
      api.get("/annonces/mes-annonces/"),      // جلب إعلاناتي فقط
      api.get("/accounts/locataires/")          // جلب قائمة المستأجرين
    ])
  ...
  try {
    ...
    // استخدم localhost للتجربة (إذا كنت على نفس الكمبيوتر)
              ...
              <TableBody>
                {contrats.map((c: any) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell><Button variant="ghost" size="icon" onClick={() => downloadPdf(c.id)}><Download className="h-4 w-4" /></Button></TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
